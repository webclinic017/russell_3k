...
class The_Portfolio_Optimizer(object):
    ...
    def __init__(self, day, return_files=True, graph_it=True):
    ...
    def optimize(self, initial_investment, num_portfolios, max_allocations, goods, graphit=False):
        ...
        risk_free_rate = 0.0178  # AS OF 2022.03.07
        ...
        def display_simulated_ef_with_random(mean_returns, cov_matrix, num_portfolios, risk_free_rate):
            ...
            rank = []
            [rank.append(x)for x in range(1, len(max_sharpe_allocation_df["ticker"]) + 1)]
            ...
            def storage_a():
                ...
                st.dataframe(min_vol_allocation_df.set_index("ticker"))
            storage_a()

            roll_out_list_a = []
            roll_out_list_a.append("Maximum Sharpe Ratio Portfolio Allocation")
            roll_out_list_a.append(
                f" - Total Stocks Allocated: [{len(max_sharpe_allocation_df.ticker)} / {self.port_count}]"
            )
            ...
            # roll_out_list_a.append(f" - Total Stocks Allocated: [{len(min_vol_allocation_df.ticker)} / {self.port_count}]")
        ...
        def display_calculated_ef_with_random(mean_returns, cov_matrix, num_portfolios, risk_free_rate):
            ...
            max_sharpe_allocation_df = pd.DataFrame(
                zip(temp_ticker, temp_allocation), columns=["ticker", "allocation"]).sort_values("allocation", ascending=False
            )
            t_lst = []
            for t in list(max_sharpe_allocation_df["allocation"]):
                if t > self.max_allocations:
        ...
        def display_ef_with_selected(mean_returns, cov_matrix, risk_free_rate):
            ...
            roll_out_list_c.append(f" - Total Stocks Allocated: [{len(max_sharpe_allocation_df.ticker)} / {self.port_count}]")
            ...
            roll_out_list_c.append(f" - Total Stocks Allocated: [{len(min_vol_allocation_df.ticker)} / {self.port_count}]")
        ...
        if self.return_files:
            for r in range(len(saver_lst_1)):
                ...
                fd.to_pickle(self.reports_portfolio / f"{namer_lst_1[r]}.pkl")
